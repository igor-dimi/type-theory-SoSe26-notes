...
> Assuming the type judgments in $\Gamma$ (the context) and that the additional variable $x$ has type $A$, 
> then the term $t$ has the type $B$ in the same context $\Gamma$. 
...
> if $t$ has type $B$ under temporary assumptions $x : A$ and context $\Gamma$, then $\labs{x}{t}$ has type $A \to B$
...
we can type check this in Rocq to verify that the type of this term is indeed $\Nat \to \Nat$:
...
In Rocq, there is a deep analogy between ordinary functions and theorem with parameters.
...
can be understood as a type. Its inhabitants are proofs of that proposition.
So if we have
...
This is a dependent function type. It is called dependent becaus the result type depends on
the input `b`. 
For example, if we apply `negb_inv` to `true`, we get:
...
The important point is that `negb_inv true` is **not** a Boolean value. It is a proof
object. More precisely, it s a proof of the proposition.
...
The tactic `destruct` performs case analysis on an object according to the possible ways
that object could have been constructed.
...
The word `destruct` is therefore connected to the idea of deconstructing a value: we inspect
its outermost constructor and split into the corresponding cases.

The same idea applies to other inductive types. For natural numbers, a value of type
`nat` is constructed either as `0` or as the successor of antoher natural nubmer. 
Therefore `destruct n.` splits the proof into the cases
...
$\lapp{1}{3}$ something else $\labs{a}{b}$
